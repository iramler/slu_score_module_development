...
ggplot(WMLAX, aes(x = faceoff_win_pct, y = win_loss_pctg, color = Division)) +
  ...
  labs(title = "Polynomial Regression: Draw Wins vs. Game Wins",
       x = "Draw Win Percentage",
       y = "Game Win Percentage") +
  theme_minimal()
```
...
- The interaction term (β = 0.9454, p = 0.0084) suggests that for women’s teams, the effect of faceoff/draw win percentage on game win percentage is 0.9454% higher than for men’s teams.
draw percentage and win percentage relationship 
make a plot that shows it, what type model is best 
regression model 
...
assess regression line 

next: bring men in 

module to teach people the need for indicator variables